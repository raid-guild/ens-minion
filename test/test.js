...
describe("MinionSubdomainRegistrar", function() {
  ...
  it('should fail to deregister a subdomain (not owner/minion)', async function() {
    await expect(minionRegistrar.connect(accounts[1]).deregister(sha3('guildname'), 'member2', resolver.address)).to.be.revertedWith('')
    expect(await ens.owner(namehash.hash('member2.guildname.eth'))).to.equal(accounts[2]._address)
  })

  it('should deregister a subdomain via subdomain owner', async function() {
    await minionRegistrar.connect(accounts[2]).deregister(sha3('guildname'), 'member2', resolver.address)
    expect(await ens.owner(namehash.hash('member2.guildname.eth'))).to.equal(NULL_ADDRESS)
  })

  it('should fail to deregister a subdomain (not registered)', async function() {
    await expect(minionRegistrar.connect(accounts[2]).deregister(sha3('guildname'), 'member2', resolver.address)).to.be.revertedWith('')
    expect(await ens.owner(namehash.hash('member2.guildname.eth'))).to.equal(NULL_ADDRESS)
  })
  ...
  it('should fail to deregister a subdomain (unlisted)', async function() {
    await expect(minionRegistrar.connect(accounts[1]).deregister(sha3('guildname'), 'member', resolver.address)).to.be.revertedWith('')
    expect(await ens.owner(namehash.hash('member.guildname.eth'))).to.equal(accounts[1]._address)
  })
  ...
  it('should register a subdomain via minion', async function() {
  ...
  it('should deregister a subdomain via minion', async function() {
    const encodedHexData = minionRegistrarInterface.functions.deregister.encode([sha3('guildname'), 'member', resolver.address])
    await minion.proposeAction(minionRegistrar2.address, 0, encodedHexData, 'Deregister member.guildname.eth')
    await moloch.sponsorProposal(3)

    await timeTravel(120)
    await moloch.submitVote(3, 1)
    await moloch.connect(accounts[1]).submitVote(3, 1)
    await moloch.connect(accounts[2]).submitVote(3, 1)

    await timeTravel(1200)
    await moloch.processProposal(3)

    await minion.executeAction(3)
    expect(await ens.owner(namehash.hash('member.guildname.eth'))).to.equal(NULL_ADDRESS)
  })
})
